...
1-> http://localhost:3000/users/signup
2-> http://localhost:3000/users/login
3->http://localhost:3000/blogs (GET REQUEST)
4->http://localhost:3000/blogs (POST REQUEST)
5->http://localhost:3000/blogs/:id (DELETE REQUEST)
6->http://localhost:3000/blogs/:id (PATCH REQUEST)
7->http://localhost:3000       (GET REQUEST)


**How to use this api**

1->when we make POST request through post man to http://localhost:3000/users/signup this url and provide data in  
josn format ex {
...
2->when we make POST request through post man to http://localhost:3000/users/login this url and provide data in  
josn format ex {
    "email":"power@gmail.com",
    "password": "777777"
}
...
3->  when we make a GET request through post man to http://localhost:3000/blogs api will show all the blogs
in our database

4-> when we make a POST request through post man to http://localhost:3000/blogs and add our jwt token 
...
5-> when we make a DELETE request through post man to http://localhost:3000/blogs/:id and add our jwt token 
in headers in Authorization field ex.(Authorization:Bearer  token) [NOTE bearer should be there in front of token]
this will delete   a blog int  our database corresponding to the given id


6-> when we make a PATCH request through post man to http://localhost:3000/blogs/:id and add our jwt token 
in headers in Authorization field ex.(Authorization:Bearer  token) [NOTE bearer should be there in front of token]
and providing data in json ex({"title":"My College","content": "IIITDM"}) this will update    a blog int  our database
corresponding to the given id
[NOTE :only authorized users with jwt token can do this operation]

7-> when we make a POST request through post man to http://localhost:3000 api will greet us showing message "Hello this is a blog api"
